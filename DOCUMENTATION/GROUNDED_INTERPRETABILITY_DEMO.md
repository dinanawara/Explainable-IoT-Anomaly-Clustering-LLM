...
> "K-Means identified three distinct attack clusters in the 5,000 most anomalous flows. Cluster 0 (31.88%, n=1,594) was characterized by concentrated HTTP traffic to port 80 with LOF anomaly scores peaking at 46.02. Domain expert analysis via GPT-4-based interpretation identified this as command and control (C2) communication typical of Zeus-family botnets. Cluster 1 (34.00%, n=1,700) showed similar patterns but with notably higher data payload sizes (max 4,062 vs 2,453 bytes), suggesting post-compromise data exfiltration. All 100% of samples in both clusters were labeled as true attacks, validating the unsupervised clustering against ground truth."

### Contribution Statement

> "We introduced **persona-based LLM interpretation** as a novel approach to provide grounded, multi-perspective analysis of anomaly clusters. By constraining LLM outputs to extracted features (no generation of new data) and obtaining consensus across 4 domain experts, we demonstrate that LLM-driven interpretation can provide interpretability without sacrificing scientific rigor."
...
├── llm_multi_persona_analysis.json        ← All 4 persona interpretations
└── NOTEBOOKS/
    └── Anomaly_Clustering_Analysis.ipynb  ← Reproducible analysis pipeline
```

**Verification**: Users can:
1. Load cluster_profiles.json to verify all quantitative stats
2. Load llm_multi_persona_analysis.json to verify LLM quotes
3. Re-run NOTEBOOKS/Anomaly_Clustering_Analysis.ipynb to reproduce clustering
